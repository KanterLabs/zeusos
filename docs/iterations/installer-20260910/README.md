...
| Zeus update and retained rollback | VM118 installed the released signed `git-eaabf4da5796` artifact through the native updater, booted Fedora and Zeus, rolled back to retained `git-4b8e9fae3ae6`, booted both choices again, and rolled forward. The final signed `git-97da2a1116c4` fix then installed and booted. GPT, Fedora ESP/boot/BLS and home fixture, Zeus ESP/home sentinel, and Temp state matched across phases. [Released receipt](vm118-released-update-rollback.json), [snapshot helper](vm118-release-update-snapshot.sh). |
...
update, a released signed Zeus update/rollback/roll-forward, the final signed
update, and the menu-only removal rehearsal. Both Fedora and Zeus boot choices
passed after the transitions. Independent Fedora and Zeus home fixtures,
Temp's absent/default Zeus policy, both ESPs, Fedora `/boot`/BLS, and the GPT
matched the baseline. The lifecycle row is accepted by the
[released receipt](vm118-released-update-rollback.json); the earlier VM117
synthetic receipt remains historical only.
